...
function Harness(){const [symbol,setSymbol]=useState("CONTROL");selectTicker=setSymbol;const state=useMemo(()=>{const config=createTestPaneConfig("/tmp/unused-risk-report",{instanceId:"risk:audit",paneId:"risk-factors",binding:{kind:"fixed",symbol}});const state=createInitialState(config);state.tickers=new Map([[symbol,createTestTicker(symbol)]]);state.focusedPaneId="risk:audit";return state;},[symbol]);const runtime=useMemo(()=>createStatefulTestPluginRuntime(),[]);const host=useRendererHost();useEffect(()=>{const original=host.openExternal;host.openExternal=async(url:string)=>{opened.push(url);};return()=>{host.openExternal=original;};},[host]);return <TestPaneProvider state={state} dispatch={()=>{}} paneId="risk:audit" pluginId="ticker-research" runtime={runtime}><PaneFooterProvider>{(footer:any)=><Box width={100} height={24} flexDirection="column"><Box height={23}><RiskFactorsPane focused width={100} height={23}/></Box><PaneFooterBar footer={footer} focused width={100}/></Box>}</PaneFooterProvider></TestPaneProvider>;}
...
test("stale discovery is disclosed; standard refresh follows latest until an explicit historical selection",async()=>{
 ...
 unavailable=false;await key("r");const latest=capture("discovery-recovered");expect(latest).toContain("Only 2026 risk analysis");expect(latest).toContain("Filed 2026-02-03");expect(latest).toContain("Report updated 2026-02-04");expect(latest).not.toContain("Discovery unavailable");
