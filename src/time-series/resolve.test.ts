...
describe("resolveChartSpecData", () => {
  ...
  test("qualified price charts retain quote metadata without injecting the fetched snapshot into history", async () => {
    ...
    const provider = createTestDataProvider({
      getQuote: async () => { quoteCalls++; return { symbol: "NESN", listingExchangeName: "SWX", currency: "CHF", instrumentType: "EQUITY", price: 999, change: 0, changePercent: 0, lastUpdated: Date.parse("2026-01-16") }; },
  ...
  test("a currency-only live quote still loads security type without replacing its price", async () => {
    ...
    const provider = createTestDataProvider({
      getQuote: async () => { quoteCalls++; return { symbol: "NVDA", listingExchangeName: "NASDAQ", currency: "USD", instrumentType: "Common Stock", price: 999, change: 0, changePercent: 0, lastUpdated: Date.parse("2026-01-16T16:00:00Z") }; },
      getTickerFinancials: async () => { financialCalls++; return emptyFinancials(); },
      getPriceHistory: async () => history,
      getPriceHistoryForResolution: async () => history,
    });
